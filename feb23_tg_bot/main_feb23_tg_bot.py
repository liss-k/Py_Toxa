...
Чтобы получить доступ к зонам развлечений (их 2: плойка и алкоголь), тебе с командой нужно пройти 7 станций.
...
Если не складывается даже с подсказкой, обратись в службу поддержки. На проводе сегодня: Саша Разгуляева, Лена Гайван и Алина Жукова - они поддержат...возможо даже помогут
...
Чтобы прислать мне кодовое слово, нажми кнопку "Ответ", выбери "1" (это номер задания) и скинь свой вариант.
...
def theGame(bot,update,job_queue,text,bot_user_base):
    try:
        ...
        ,'миньон в красной зоне' # 'm1' - стулья, место Лены Коваленко
...
И только потом вводи номер этапа.
            '''            
                ...
                custom_keyboard = [['Ответ', 'Подсказка']]
                reply_markup = ReplyKeyboardMarkup(custom_keyboard, resize_keyboard=True, one_time_keyboard=False)            
                return chatSticker(bot,update,custom_keyboard)
...
И в итоге из букв получится..........
            ...
            answer_dict = {
                '1' : {'1' : ['путь'], #двойное дно, одинаковое
                       '2' : ['тигр'],
                       '3' : ['пуск'],
                       '4' : ['гром'],
                       '5' : ['трюк'],
...
Место: {}
Иди же скорее туда и покажи это сообщение ведущей на станции, а то не пустят ;)
И не забудь чек-лист'''.format(team, place_name)
...
def backFunc():
    angry_list=['Ты хочешь дать ответ или получить подсказку?😑 '
    ,'Ты хочешь дать ответ или получить подсказку?😶'
    ,'Ты хочешь дать ответ или получить подсказку?🙄']
...
def wrongAnswer(bot,update,custom_keyboard):
    ...
    general_answer=['Я смотрю, интеллектуальные игры - не твоя сильная сторона... Попробуй ещё раз'
    ,'Хлебушек вместо мозга? Попробуй ещё раз'
    ,'Как думаешь, у тебя или у пшеницы выше IQ? Попробуй ещё раз'
    ,'Мысли как мюсли, а речь как кисель... Попробуй ещё раз'
    ,'Я где-то слышала, что у мужчин есть логика🤔 Попробуй ещё раз'
    ,'Попки младенцев напоминают мне от твоём мозг. Попробуй ещё раз'
    ,'Бооожееее...ты наугад тут что ли пишешь? Попробуй ещё раз'
    ,'Невежество порождает собственную истину... Попробуй ещё раз'
    ]
